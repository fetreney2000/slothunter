...
<div class="flex min-h-screen items-center justify-center bg-gradient-to-br from-surface-50 to-surface-200-700-token p-4">
	<div class="card w-full max-w-md p-8 space-y-6">
		...
		<hr class="hr" />
		{#if error}
			<div class="alert preset-filled-error-500"><p class="text-sm">{error}</p></div>
		{/if}
		<form onsubmit={handleLogin} class="space-y-4">
			...
			<button type="submit" class="btn preset-filled-primary-500 w-full text-lg" disabled={loading}>
